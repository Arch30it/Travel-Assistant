...
# Set page configuration with modern theming
st.set_page_config(page_title="TravelBuddy", page_icon="🌍", layout="wide")

# Load environment variables
def configure():
    dotenv_path = '/Users/architmurgudkar/Documents/MDSI/NLP/Assignment2-TravelBuddy/TravelBuddy/py.env'
...
# Handle chat response with translation
...
st.markdown("<h1 style='text-align: center; color: #4CAF50;'>🌍 TravelBuddy</h1>", unsafe_allow_html=True)
...
if st.session_state.chat_history:
    ...
    for entry in st.session_state.chat_history:
        if entry['type'] == 'Question':
            ...
            st.markdown(f"<div style='background-color: #e6f7ff; padding: 10px; border-radius: 5px;'><strong>🔵 TravelBuddy:</strong> {entry['content']}</div>", unsafe_allow_html=True)
